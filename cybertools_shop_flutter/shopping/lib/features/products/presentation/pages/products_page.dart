...
class _ProductsPageState extends State<ProductsPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Cybersecurity Tools'),
        actions: const [
          CartIconButton(),
        ],
        ...
        builder: (context, state) {
          return state.when(
            initial: () => const Center(
              child: Text('Type a pattern to search tools (e.g. "*scan*" or "/^nmap/")'),
            ),
            ...
            searchSuccess: (products, query) => _ProductsList(
              products: products,
              isLoading: false,
              emptyLabel: 'No tools found for "$query"',
            ),
...
class _ProductsList extends StatelessWidget {
  const _ProductsList({
    ...
    this.emptyLabel = 'No tools found',
  });
  ...
  Widget build(BuildContext context) {
    return TextField(
      ...
      decoration: InputDecoration(
        hintText: 'Search by pattern: * ? or /regex/ ...',
